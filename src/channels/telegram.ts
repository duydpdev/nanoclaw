...
const TELEGRAM_MAIN_CHAT_JID = 'tg:8204562202';
...
const botCurrentNames = new Map<number, string>();
let nextPoolIndex = 0;
const MAX_SENDER_ASSIGNMENTS = 500;

function resetTelegramBotPoolState(): void {
  poolApis.length = 0;
  senderBotMap.clear();
  botCurrentNames.clear();
  nextPoolIndex = 0;
}
 ...
 */
export function resetTelegramBotPoolForTests(): void {
  resetTelegramBotPoolState();
  mainBotApi = null;
}
...
export async function sendPoolMessage(
  ...
  if (poolApis.length === 0) {
    if (!mainBotApi) {
      throw new Error('Telegram main bot not initialized');
    }
  ...
  if (idx === undefined) {
    ...
    if (senderBotMap.size > MAX_SENDER_ASSIGNMENTS) {
      const oldestKey = senderBotMap.keys().next().value;
      if (oldestKey) senderBotMap.delete(oldestKey);
    }
  }

  const api = poolApis[idx];
  const currentName = botCurrentNames.get(idx);
  if (currentName !== sender) {
    try {
      await api.setMyName(sender);
      botCurrentNames.set(idx, sender);
  ...
  try {
    if (text.length <= maxLength) {
      ...
      for (let i = 0; i < text.length; i += maxLength) {
        await sendTelegramMessage(api, numericId, text.slice(i, i + maxLength));
      }
    }
    logger.info(
...
export class TelegramChannel implements Channel {
  ...
  private notifiedUnregisteredChats = new Set<string>();
  ...
  private async notifyMainForUnregisteredChat(
    chatJid: string,
    chatName: string,
    senderName: string,
    sender: string,
    ...
    if (!this.bot) return;
    if (this.notifiedUnregisteredChats.has(chatJid)) return;

    const mainChatEntry = Object.entries(this.opts.registeredGroups()).find(
      ([, group]) => group.isMain === true,
    );
    const mainChatJid = mainChatEntry?.[0] || TELEGRAM_MAIN_CHAT_JID;

    this.notifiedUnregisteredChats.add(chatJid);
    const mainChatId = mainChatJid.replace(/^tg:/, '');

    const message = [
      '🚨 *Telegram chat chưa được duyệt đang nhắn bot*',
      `• chatId: \`${chatJid}\``,
      `• chat name: ${chatName}`,
      `• sender: ${senderName} (${sender || 'unknown'})`,
      '',
      'Main chat hãy đăng ký/duyệt chat này trước khi dùng bot.',
    ].join('\n');

    await sendTelegramMessage(this.bot.api, mainChatId, message);
  }
  ...
  async connect(): Promise<void> {
    ...
    // Register Telegram bot commands and callbacks in a dedicated module.
    // Commands in this set are skipped in general text handler below.
    const TELEGRAM_BOT_COMMANDS = registerTelegramCommands(this.bot, this.opts);

    this.bot.on('message:text', async (ctx) => {
      const chatJid = `tg:${ctx.chat.id}`;
      const senderNameForCommand =
        ctx.from?.first_name ||
        ctx.from?.username ||
        ctx.from?.id?.toString() ||
        'Unknown';
      const senderForCommand = ctx.from?.id?.toString() || '';
      const chatNameForCommand =
        ctx.chat.type === 'private'
          ? senderNameForCommand
      ...
      if (ctx.message.text.startsWith('/')) {
        ...
        if (TELEGRAM_BOT_COMMANDS.has(cmd)) {
          ...
          if (!group) {
            await this.notifyMainForUnregisteredChat(
              chatJid,
              chatNameForCommand,
              senderNameForCommand,
              senderForCommand,
      ...
      if (!group) {
        ...
        await ctx.reply(
          'Chat này chưa được duyệt. Vui lòng gửi chat ID cho admin/main chat để đăng ký trước khi dùng bot.',
        );
        logger.debug(
          { chatJid, chatName },
